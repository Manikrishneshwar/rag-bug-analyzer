...
sdk: streamlit
sdk_version: "1.40.1"
app_file: deployed/app.py
pinned: false
---

<!-- The YAML block above is Hugging Face Spaces config — it tells Spaces
which SDK/entry file to run. It's harmless on GitHub/anywhere else;
Spaces reads it, everyone else just sees normal markdown below. -->
...
- **Synthesis**: pluggable via `LLM_PROVIDER` in `.env` — **Groq**
  (`llama-3.1-8b-instant` by default, free tier, no cost — this is the
...
works on a fresh, ephemeral deployment with no extra setup step.

1. **Create the Space**: go to [huggingface.co/new-space](https://huggingface.co/new-space)
   (free account, no card). Pick a name, set **SDK = Streamlit**, visibility
   your choice, then create it.
2. **Push this repo to it**: every Space is its own git remote. From this
   project folder:
   ```bash
   git remote add space https://huggingface.co/spaces/<your-username>/<space-name>
   git push space main
   ```
   (Hugging Face will prompt for a username + an access token as the
   password the first time — generate one at
   [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens),
   "write" access is enough.)
3. **Add your Groq key as a Secret** (not committed to git): in the Space's
   page → Settings → **Variables and secrets** → add secret
   `GROQ_API_KEY` with your key. Add `GROQ_MODEL` too if you've changed it
   from the `.env.example` default.
4. The Space rebuilds automatically on push — first build takes a few
   minutes (installing PyTorch etc.), then the index auto-builds on the
   app's first request. After that you have a public URL like
   `https://huggingface.co/spaces/<your-username>/<space-name>` to put on
   your resume.

The README's YAML frontmatter at the very top of this file is Space
config (title/emoji/SDK/entry point) — Spaces reads it automatically,
everywhere else it's just an HTML comment-adjacent block that renders as
nothing unusual.
